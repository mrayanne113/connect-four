  ...
  checkUpperRightDownLeft (column, row) {
    ...
    if (column <= 3 && row >= 2) {
      ...
      for (let i = column; i < 6; i++) {
        tempRow--;
        console.log('Upper right - testing row: ', tempRow, 'testing column ', i + 1);
    ...
    if (column >= 1 && row >= 2) {
      ...
      for (let j = column; j > 1; j--) {
        tempRow++;
        if (tempRow <= 6 && tempRow > 1) {
          console.log('Down Left - testing row: ', tempRow, 'testing column ', j - 1);
    ...
    for (let a = 0; a < finalArr.length; a++) {
      console.log('numOfCheckers', numOfCheckers);
